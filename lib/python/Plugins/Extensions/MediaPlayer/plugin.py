...
class MediaPlayer(Screen, InfoBarBase, InfoBarScreenSaver, InfoBarSeek, InfoBarAudioSelection, InfoBarAspectSelection, InfoBarCueSheetSupport, InfoBarNotifications, InfoBarSubtitleSupport, HelpableScreen, InfoBarResolutionSelection):
    ...
    def __init__(self, session, args = None):
        ...
        self['OkCancelActions'] = HelpableActionMap(self, 'OkCancelActions', {'ok': (self.ok, _('Add file to playlist')),
         'cancel': (self.exit, _('Exit mediaplayer'))}, -2)
        self['MediaPlayerActions'] = HelpableActionMap(self, 'MediaPlayerActions', {'play': (self.xplayEntry, _('Play entry')),
         ...
         'subtitles': (self.subtitleSelection, _('Subtitle selection'))}, -2)
        self['InfobarEPGActions'] = HelpableActionMap(self, 'InfobarEPGActions', {'showEventInfo': (self.showEventInformation, _('show event details'))})
        self['actions'] = MoviePlayerActionMap(self, ['DirectionActions'], {'right': self.rightDown,
    ...
    def previousMarkOrEntry(self):
        if not self.jumpPreviousNextMark(lambda x: -x - 450000, start=True):
            next = self.playlist.getCurrentIndex() - 1
            if next >= 0:
                self.changeEntry(next)
            ...
            if self.currList == 'playlist':
                if self.seekstate == self.SEEK_STATE_PAUSE:
                    if self.playlist.getCurrentIndex() != self.playlist.getSelectionIndex():
                        self.changeEntry(self.playlist.getSelectionIndex())
                    elif self.ext in AUDIO_EXTENSIONS or self.isAudioCD:
                    ...
                    if self.playlist.getCurrentIndex() != self.playlist.getSelectionIndex():
                        self.changeEntry(self.playlist.getSelectionIndex())
                    elif not self.playlist.isStopped():
                        if self.ext in AUDIO_EXTENSIONS or self.isAudioCD:
                            self.pauseService()
                        else:
                            self.hideAndInfoBar()
                    ...
                    self.playEntry()
        elif self.currList == 'playlist' and self.seekstate == self.SEEK_STATE_PAUSE:
            self.playEntry()
        elif self.isStateForward(self.seekstate) or self.isStateBackward(self.seekstate):
            self.playEntry()
        else:
...
def filescan(**kwargs):
    ...
    return [Scanner(mimetypes=['video/mpeg',
      'video/MP2T',
      'video/x-msvideo',
      'video/mkv'], paths_to_scan=[ScanPath(path='', with_subdirs=False), ScanPath(path='PRIVATE/AVCHD/BDMV/STREAM', with_subdirs=False)], name='Movie', description=_('Watch movies...'), openfnc=movielist_open),
     Scanner(mimetypes=['video/x-vcd'], paths_to_scan=[ScanPath(path='mpegav', with_subdirs=False), ScanPath(path='MPEGAV', with_subdirs=False)], name='Video CD', description=_('View video CD...'), openfnc=filescan_open),
     Scanner(mimetypes=['audio/mpeg',
      'audio/x-wav',
      'application/ogg',
      'audio/x-flac'], paths_to_scan=[ScanPath(path='', with_subdirs=False)], name='Music', description=_('Play music...'), openfnc=filescan_open),
     Scanner(mimetypes=['audio/x-cda'], paths_to_scan=[ScanPath(path='', with_subdirs=False)], name='Audio-CD', description=_('Play audio-CD...'), openfnc=audioCD_open)]


def Plugins(path, **kwargs):
    desc_mainmenu = PluginDescriptor(name=_('Media player'), description=_('Play back media files'), where=PluginDescriptor.WHERE_MENU, needsRestart=False, fnc=menu)
    desc_mainmenu2 = (PluginDescriptor(name=_('Media player'), where=PluginDescriptor.WHERE_MENU, needsRestart=False, fnc=audiocdscan),)
    desc_pluginmenu = PluginDescriptor(name=_('Media player'), description=_('Play back media files'), where=PluginDescriptor.WHERE_PLUGINMENU, icon='MediaPlayer.png', needsRestart=False, fnc=main)
    desc_filescan = PluginDescriptor(name=_('Media player'), where=PluginDescriptor.WHERE_FILESCAN, needsRestart=False, fnc=filescan)
    list = []
    list.append(desc_filescan)
    if config.mediaplayer.onPluginList.value:
        list.append(desc_pluginmenu)
    if config.mediaplayer.onMainMenu.value:
        list.append(desc_mainmenu)
    list.append(desc_mainmenu2)
    return list
